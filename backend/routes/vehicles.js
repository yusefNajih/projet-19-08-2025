...
router.put('/:id/admin-status', [auth, authorize('admin')], async (req, res) => {
  try {
...
router.get('/', auth, [
  ...
  query('status').optional().isIn(['available', 'rented', 'maintenance', 'out_of_service']).withMessage('Invalid status'),
  query('brand').optional().isString().trim(),
  query('fuelType').optional().isIn(['essence', 'diesel', 'electric', 'hybrid']).withMessage('Invalid fuel type')
], async (req, res) => {
  try {
...
router.post(
  ...
  [
    ...
    body('licensePlate').notEmpty().trim().withMessage('License plate is required'),
    body('fuelType').isIn(['essence', 'diesel', 'electric', 'hybrid']).withMessage('Invalid fuel type'),
    body('dailyPrice').isFloat({ min: 0 }).withMessage('Daily price must be a positive number'),
    ...
    body('seats').optional().isInt({ min: 2, max: 50 }).withMessage('Seats must be between 2 and 50')
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      ...
      const {
        brand, model, year, licensePlate, fuelType, dailyPrice, mileage,
        color, transmission, seats, notes
      } = req.body;
...
router.put('/:id', [auth, authorize('admin', 'manager')], [
  ...
  body('model').optional().notEmpty().trim().withMessage('Model cannot be empty'),
  body('year').optional().isInt({ min: 1900, max: new Date().getFullYear() + 1 }).withMessage('Invalid year'),
  body('licensePlate').optional().notEmpty().trim().withMessage('License plate cannot be empty'),
  body('fuelType').optional().isIn(['essence', 'diesel', 'electric', 'hybrid']).withMessage('Invalid fuel type'),
  body('dailyPrice').optional().isFloat({ min: 0 }).withMessage('Daily price must be a positive number'),
  body('mileage').optional().isInt({ min: 0 }).withMessage('Mileage must be a positive number'),
  body('status').optional().isIn(['available', 'rented', 'maintenance', 'out_of_service']).withMessage('Invalid status'),
  body('color').optional().trim(),
  body('transmission').optional().isIn(['manual', 'automatic']).withMessage('Invalid transmission'),
  body('seats').optional().isInt({ min: 2, max: 50 }).withMessage('Seats must be between 2 and 50')
], async (req, res) => {
...
// @access  Private
router.get('/:id/check-availability', auth, [
  ...
  try {
    ...
    const Reservation = require('../models/Reservation');
    const conflict = await Reservation.checkConflict(vehicleId, new Date(startDate), new Date(endDate));
